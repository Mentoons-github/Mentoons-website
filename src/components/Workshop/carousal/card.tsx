  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4 xs:mb-6 gap-4 lg:gap-6">
          <motion.h2
            ...
            {workshop.ageGroups
              ...
              .map((ageGroup, index) => (
                <motion.img
                  ...
                  className="h-16 w-16 xs:h-20 xs:w-20 sm:h-24 sm:w-24 md:h-32 md:w-32 lg:h-40 lg:w-40 xl:h-48 xl:w-48 
                           object-cover rounded-lg xs:rounded-xl 
        ...
        <motion.div
          className="flex flex-col xs:flex-row flex-wrap justify-center lg:justify-start 
                   gap-2 xs:gap-3 sm:gap-4 mt-6 xs:mt-8"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
        >
          {(["overview", "ageGroups", "whyChoose"] as const).map(
            (section, index) => (
              <motion.button
                key={section}
                onClick={() => setActiveSection(section)}
                className={`px-4 xs:px-5 sm:px-6 
                ...
                whileHover={{ scale: activeSection === section ? 1.05 : 1.02 }}
                whileTap={{ scale: 0.98 }}
          ...
          {workshop.workshopName.toLowerCase().includes("career corner") && (
            <motion.button
