...
const FooterLinks = ({
  ...
  return (
    <div className="w-full md:w-fit space-y-3 md:space-y-5 text-center md:text-left">
      <h1 className="forum text-lg sm:text-xl md:text-2xl text-[#1A1D3B] tracking-[1.5px] md:tracking-[2.4px]">
        {label}
      </h1>
      <ul className="space-y-2 md:space-y-5">
        {Links.map((item, index) => (
          <li
            key={index}
            className="flex justify-center md:justify-start items-center gap-2 md:gap-3 mulish"
          >
            <Link
