...
const StatCard = ({
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white/70 backdrop-blur-sm shadow-xl rounded-2xl p-6 border border-white/20 cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-2xl"
...
const DashboardAnalytics = () => {
  ...
  const [revenueCategory, setRevenueCategory] = useState("workshops");
  const [revenueSubCategory, setRevenueSubCategory] = useState("instant-katha");
  ...
  const getTotalRevenue = () => {
    const currentData = getCurrentRevenueData();
    return currentData.reduce((sum, item) => sum + item.revenue, 0);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200">
              ...
              <div className="text-3xl font-bold text-blue-900">
                ₹
                {Math.round(
                  getTotalRevenue() / getCurrentRevenueData().length
                ).toLocaleString()}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-6 border border-purple-200">
              ...
              <div className="text-3xl font-bold text-purple-900">
                ₹
                {Math.max(
                  ...getCurrentRevenueData().map((d) => d.revenue)
                ).toLocaleString()}
              </div>
            </div>
