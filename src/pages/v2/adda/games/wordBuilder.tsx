...
import WordBuilderLobby from "@/components/adda/game/wordBuilder/lobby";
...
import WordBuilderPlayzone from "@/components/adda/game/wordBuilder/playzone";
import { GAME_DATA } from "@/constant/adda/game/wordBuilder";
...
const WordBuilder = () => {
  ...
  const gameId = `word_builder_${difficulty}`;
  ...
  const selectRounds = useMemo(() => {
    const round = GAME_DATA[difficulty] || GAME_DATA.easy;
    return [...round].sort(() => Math.random() - 0.5).slice(0, 2);
  }, [difficulty]);
  ...
  return (
    <>
      ...
      {currentState === "lobby" && (
        <WordBuilderLobby showDifficultyModal={() => setIsModalOpen(true)} />
      )}
      ...
      {currentState === "result" && (
        <ResultScreen
          score={finalScore}
          difficulty={difficulty}
          totalRounds={selectRounds.length}
          onPlayAgain={handlePlayAgain}
          goToLobby={goToLobby}
          resultMessage={
            finalScore > 0
              ? "You're a word master!"
              : "Try again to find more words!"
          }
        />
      )}
