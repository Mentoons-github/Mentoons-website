import MindMathLobby from "@/components/adda/game/mathMind/lobby";
...
import PlayerZoneMindMath from "@/components/adda/game/mathMind/playerZone";
import { generateRounds } from "@/utils/game/mathMind";
...
const MindMath = () => {
  ...
  const gameId = `mindmath_${difficulty}`;
  ...
  const currentRounds = useMemo(() => {
    return generateRounds(difficulty);
  }, [difficulty]);
  ...
  return (
    <>
      ...
      {currentState === "lobby" && (
        <MindMathLobby showDifficultyModal={() => setIsModalOpen(true)} />
      )}
      {currentState === "play" && (
        <PlayerZoneMindMath
